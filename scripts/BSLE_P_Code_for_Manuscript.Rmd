...
leach.Pnorm.conc<-ggplot(leach.fig.stack, aes(x = Burn_Severity, y = value, fill = Land_Coverage_Category)) +
  ...
  xlab("Burn Severity") +
  scale_fill_manual(values = c("#346648", "#D8B70A")) +
...
#carbon
model<-aov(log10(Solid_C_mg_kg)~Burn_Severity*Land_Coverage_Category, data = data_solids)
summary(model)
#model assumptions
hist(log10(data_solids$Solid_C_mg_kg))
ggqqplot(residuals(model))
#check normality of residuals 
shapiro.test(log10(data_solids$Solid_C_mg_kg)) #p > 0.05 so can assume normality 
#check homogeneity of variance assumption (homoscedastcity)
plot(fitted(model),residuals(model))
performance::check_heteroscedasticity(model) #p > 0.05 so can assume homoscedastic

#nitrogen
model<-aov(log10(Solid_N_mg_kg)~Burn_Severity*Land_Coverage_Category, data = data_solids)
summary(model)
#model assumptions
hist(log10(data_solids$Solid_N_mg_kg))
ggqqplot(residuals(model))
#check normality of residuals 
shapiro.test(log10(data_solids$Solid_C_mg_kg)) #p > 0.05 so can assume normality 
#check homogeneity of variance assumption (homoscedastcity)
...
#is C:P or N:P influenced by burn severity or land cover?
stoich.stat.C<-subset(stoich, variable=="C:P" & filter=="Solid")
stoich.stat.N<-subset(stoich, variable=="N:P" & filter=="Solid")

model<-aov(log10(value)~Burn_Severity*Land_Coverage_Category, data = stoich.stat.C)
summary(model)
#model assumptions
hist(stoich.stat.C$value)
ggqqplot(residuals(model))
#check normality of residuals 
shapiro.test(log10(stoich.stat.C$value)) #p > 0.05 so can assume normality 
#check homogeneity of variance assumption (homoscedastcity)
plot(fitted(model),residuals(model))
performance::check_heteroscedasticity(model) #p > 0.05 so can assume homoscedastic

model<-aov(log10(value)~Burn_Severity*Land_Coverage_Category, data = stoich.stat.N)
summary(model)
#model assumptions
hist(stoich.stat.N$value)
ggqqplot(residuals(model))
#check normality of residuals 
shapiro.test(log10(stoich.stat.N$value)) #p > 0.05 so can assume normality 
#check homogeneity of variance assumption (homoscedastcity)
plot(fitted(model),residuals(model))
performance::check_heteroscedasticity(model) #p > 0.05 so can assume homoscedastic
#interaction post hoc: 
...
```{r PCA NMR}
solid <- data_solids %>%
  select(Land_Coverage_Category, Burn_Severity, Ortho, Mono, Di, Pyro) 

pca <- prcomp(solid[3:6], center=TRUE, scale=TRUE)
...
colnames(loadings)<-c('PC1','PC2')
rownames(loadings)<-colnames(solid[3:6])
loadingstest<-as.data.frame(loadings)
pca_out <- as.data.frame(pca$x)
percentage <- round(pca$sdev^2/sum(pca$sdev^2)*100,1)
percentage <- paste(colnames(pca_out), "-", paste(as.character(percentage), "%", sep=""))
...
#stress below 0.05 is a good fit


, XANES_Po, XANES_Pi, XANES_Fe, XANES_Na, XANES_Ca, XANES_Al, XANES_Mg, XANES_K, XANES_Pi_Fe, XANES_Pi_Na, XANES_Pi_Ca, XANES_Pi_Al, XANES_Pi_Mg, XANES_Pi_K, XANES_Po_Fe, XANES_Po_Na, XANES_Po_Ca, XANES_Po_Al, XANES_Po_Mg, XANES_Po_K, Solid_P_g_kg, Solid_Ca_g_kg, Solid_Fe_g_kg, Solid_Al_g_kg, Solid_K_g_kg, Solid_Mg_g_kg, Solid_Na_g_kg, Solid_S_g_kg, Solid_C_g_kg, Solid_N_g_kg

#NMR
solid <- data_solids %>%
  select(Ortho, Mono, Di, Pyro) 

#create dataframe for independent variables
enviro<-data_solids[c('Land_Coverage_Category', 'Burn_Severity')]


###no distance matrix, no march, GW only
...
NMR_NMS_4 <-
  metaMDS(solid,
          ...
          trymax = 500,
          wascores = TRUE)
#Run 20 stress 0.02990506 
#... Procrustes: rmse 0.003900023  max resid 0.01001928 
goodness(NMR_NMS_4)
stressplot(NMR_NMS_4)

NMRenv1 <- envfit(NMR_NMS_4, enviro, permu = 999)
 
NMR_NMS_4 %>% ordiplot(type='none') %>% points("sites") %>% ordiellipse(groups=enviro$Burn_Severity, col=c("#999933","#d7191c","#abdda4","blue") )
plot(NMRenv1, p.max = 0.05)


#XANES
solid <- data_solids %>%
  select(XANES_Pi_Fe, XANES_Pi_Na, XANES_Pi_Ca, XANES_Pi_Al, XANES_Pi_Mg, XANES_Pi_K, XANES_Po) 
