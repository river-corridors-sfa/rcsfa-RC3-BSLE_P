...
#need to add MBD stats back in

```

```{r Leachate fig}
#two part figure for leachates: total particulate and total aqueous P

#Leachable P (normalized by concentration of solid char) stacked barchart
leach.fig<-data %>%
...
leach.fig <- leach.fig %>%
  mutate(
    ...
    y = case_when(
      ...
      variable == "Aqueous Molybdate Reactive" & Land_Coverage_Category == "Douglas-fir Forest" & Burn_Severity == "Unburned" ~ 90,
      variable == "Aqueous Molybdate Reactive" & Land_Coverage_Category == "Douglas-fir Forest" & Burn_Severity == "Low" ~ 110,
      variable == "Aqueous Molybdate Reactive" & Land_Coverage_Category == "Douglas-fir Forest" & Burn_Severity == "Moderate" ~ 75,
      variable == "Aqueous Molybdate Reactive" & Land_Coverage_Category == "Douglas-fir Forest" & Burn_Severity == "High" ~ 50,
      ...
      variable == "Aqueous Molybdate Reactive" & Land_Coverage_Category == "Sagebrush Shrubland" & Burn_Severity == "Moderate" ~ 40),
...
leach.Pnorm.conc.mbd<-ggplot(leach.fig.mbd, aes(x=Burn_Severity, y=value, fill=Land_Coverage_Category))+geom_boxplot(show.legend=FALSE)+theme_classic()+ylab(expression(paste(Leachable~P~(mg~g^-1), sep="")))+ xlab("Burn Severity")+scale_fill_manual(values=c("gray", "gray"))+theme(text = element_text(size = 15), axis.text.x = element_text(angle = 90))+facet_grid(.~Land_Coverage_Category, scales = "free")

cowplot::save_plot("../figures/leach.Pnorm.conc.mbd.pdf", leach.Pnorm.conc, ncol = 1, nrow = 1, base_aspect_ratio= 1.6:2.6, dpi=100)
