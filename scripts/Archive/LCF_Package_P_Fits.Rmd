...
#all sample and standard spectra were averaged in Sam's pack, imported to Athena to determine energy shift, exported as xmu data file
#make sure to have a csv with background subtraction and normalization details for the Athena fit function
#reminder that R can't overwrite files with the same name so make sure all sample and refernce compounds have unique names
...
## Load libraries
```{r library}
##package details: https://cran.r-project.org/web/packages/LCF/LCF.pdf
...
# Changing directory to import data files
directory= "../data/XANES_Determine_RC"

# Find files that are samples
samples = list.files(path=directory, pattern = "BSLE")
stds = list.files(path=directory, pattern = "std")

# Read in different background correction and normalization values for stds and samples

val = read.csv("../data/XANES_Determine_RC/File_BS_Norm_Parameters.csv")
#names(val)[1] = "File_Name" # correcting the column name as it had one issue
...
# Plots
  ...
  pdf("../results/XANES_Determine_RC_Output/plots/","Baseline_Correction_",stds[i],".pdf") 
...
# Plots
 pdf("../results/XANES_Determine_RC_Output/plots/","Baseline_Correction_",samples[i],".pdf")     
...
##Determine probable reference compounds in samples to narrow down the number used in the Athena Program fits
```{r athena, results=F, message=F, warning=F}
# Standards 
# Changing working directory so the Athena outputs go to a different folder 
setwd(paste0(wd,"Athena_output"))

## exclude portions smaller 5% = 0.05
#I tried adding a file output (to hopefully use for plotting) and also printing more than just the best fit
...
write.csv(athena.final,paste0(getwd(),"/Fit_using_athena_fit_excluding_portions_results.csv"),row.names = T)
#write.csv(athena.final, file="Fit_using_athena_fit_excluding_portions_results.csv")
...
pdf(paste0(getwd(),"/Athena_output/plots/Athena_LCF_",sample.name2,".pdf"))
par(pty="s")
...
sample.name2 = gsub(".xmu","",sample.name)
athena.fit.exclude$pre.adj.1[grep(sample.name2,rownames(athena.fit.exclude)):(grep(sample.name2,rownames(athena.fit.exclude))+19)] = samples.val$BS_1[which(samples.val$File_Name==sample.name)]
athena.fit.exclude$pre.adj.2[grep(sample.name2,rownames(athena.fit.exclude)):(grep(sample.name2,rownames(athena.fit.exclude))+19)]= samples.val$BS_2[which(samples.val$File_Name==sample.name)]
athena.fit.exclude$post.adj.1[grep(sample.name2,rownames(athena.fit.exclude)):(grep(sample.name2,rownames(athena.fit.exclude))+19)] = samples.val$Norm_1[which(samples.val$File_Name==sample.name)]
athena.fit.exclude$post.adj.2[grep(sample.name2,rownames(athena.fit.exclude)):(grep(sample.name2,rownames(athena.fit.exclude))+19)] = samples.val$Norm_2[which(samples.val$File_Name==sample.name)]
