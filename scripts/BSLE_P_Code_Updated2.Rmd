...
#MBD mg/g P in char 
model<-lmer(log10(Leachate_mg_MBD_per_g_P_in_solid_filt0.7)~Burn_Severity*Land_Coverage_Category+(1|Parent_ID), data=data) #if want to use maximum likelihood , REML=FALSE
...
#updated with leachable P norm
#ICP filtered (<0.7 nominal phase) normalized to total P in solid char
...
#ICP particulates (>0.7 nominal phase) normalized to total P in solid char
...
#ICP unfiltered leachate (>0.7 + <0.7 nominal phases) normalized to total P in solid char
model<-lmer(log10(Leachate_mg_P_per_g_P_in_solid_unfilt)~Burn_Severity*Land_Coverage_Category+(1|Parent_ID), data=data)
model.BS<-lmer(log10(Leachate_mg_P_per_g_P_in_solid_unfilt)~Land_Coverage_Category+(1|Parent_ID), data=data)
model.LC<-lmer(log10(Leachate_mg_P_per_g_P_in_solid_unfilt)~Burn_Severity+(1|Parent_ID), data=data)
model.int<-lmer(log10(Leachate_mg_P_per_g_P_in_solid_unfilt)~Burn_Severity+Land_Coverage_Category+(1|Parent_ID), data=data)
...
hist(log10(data$Leachate_mg_P_per_g_P_in_solid_unfilt))
ggqqplot(residuals(model))
plot(lmer(log10(Leachate_mg_P_per_g_P_in_solid_unfilt)~Burn_Severity*Land_Coverage_Category+(1|Parent_ID), data=data))
