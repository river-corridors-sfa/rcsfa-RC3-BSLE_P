...
saveRDS(bsle_mass_loss_forP, "~/GitHub/rcsfa-RC3-BSLE_P/data/mass_loss_data.rds") #remove me before publishing in data package, just saving this for morgan for now 
```
...
```{r summary stats}
bsle_mass_loss_forP %>% 
  group_by(Land_Coverage_Category, Burn_Severity) %>%
 summarise(mean = mean(estimate_perc_mass_loss), sd = sd(estimate_perc_mass_loss))
```
